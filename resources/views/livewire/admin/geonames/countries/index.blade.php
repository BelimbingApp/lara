                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-border-default">
                        <thead class="bg-surface-subtle/80">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">{{ __('ISO') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">{{ __('Country') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">{{ __('Capital') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">{{ __('Phone') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">{{ __('Currency') }}</th>
                                <th class="px-6 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider">{{ __('Population') }}</th>
                        ...
                        <tbody class="bg-surface-card divide-y divide-border-default">
                            @forelse($countries as $country)
                                <tr wire:key="country-{{ $country->id }}" class="hover:bg-surface-subtle/50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-ink">{{ $country->iso }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-muted">{{ $country->capital }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-muted">{{ $country->phone }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-muted">{{ $country->currency_code }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-muted text-right">{{ number_format($country->population) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-muted">{{ $country->updated_at?->format('Y-m-d') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-sm text-muted">{{ __('No countries found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
