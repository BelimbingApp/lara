...
stop_caddy() {
    ...
    if [ -f "$PROJECT_ROOT/Caddyfile.blb" ]; then
        caddyfile_path="$PROJECT_ROOT/Caddyfile.blb"
...
start_caddy() {
    ...
    # Priority: Caddyfile.blb (generated by setup) > Caddyfile (template)
    local caddyfile_path=""
    if [ -f "$PROJECT_ROOT/Caddyfile.blb" ]; then
        caddyfile_path="$PROJECT_ROOT/Caddyfile.blb"
        log "Using generated Caddyfile: $caddyfile_path"
    elif [ -f "$PROJECT_ROOT/Caddyfile" ]; then
        caddyfile_path="$PROJECT_ROOT/Caddyfile"
        log "Using template Caddyfile: $caddyfile_path"
    else
